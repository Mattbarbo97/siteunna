...
import { ThemeProvider, CircularProgress, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Typography, Box, TextField, InputAdornment, Button, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
  ...
  const handleDelete = async (paciente) => {
    ...
    if (confirmar) {
      try {
        ...
        setPacientes(pacientes.map((item) => item.id === paciente.id ? { ...item, ativo: false } : item));
  ...
  const handleAdicionarPaciente = async (dadosPaciente) => {
    try {
      ...
      <Button onClick={handleFecharModalCadastro}>Cancelar</Button>
        ...
        maxWidth="md"
      >
        <DialogTitle>{modoEdicao ? 'Editar Paciente' : 'Detalhes do Paciente'}</DialogTitle>
        <DialogContent>
          <CadastroPaciente paciente={usuarioSelecionado} modoEdicao={modoEdicao} onSalvar={handleAdicionarPaciente} />
        </DialogContent>
        <DialogActions>

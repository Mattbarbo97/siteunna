...
import { CircularProgress, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Typography, Box, Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, InputAdornment } from '@mui/material';
...
const ModalDetalhesUsuario = ({ usuario, aberto, fecharModal, modoEdicao, onSave }) => {
  ...
  return (
    <Dialog open={aberto} onClose={handleClose}>
      <DialogTitle>{modoEdicao ? 'Editar Usuário' : 'Detalhes do Usuário'}</DialogTitle>
      <DialogContent>
        {modoEdicao ? (
          <>
            <TextField
        ...
        <DialogActions>
        <Button onClick={handleClose}>{modoEdicao ? 'Salvar' : 'Fechar'}</Button>
      </DialogActions>
    </Dialog>
  );
};
...
const [modalCadastroAberto, setModalCadastroAberto] = useState(false);
 ...
 const handleFecharModalCadastro = () => {
  ...
  const handleDelete = async (usuario) => {
    ...
    if (confirmar) {
      try {
        const docRef = doc(getFirestore(), "usuarios_cadastrados", usuario.id);
  ...
  const handleSaveEdicao = async (usuarioEditado) => {
    ...
    try {
      const docRef = doc(getFirestore(), "usuarios_cadastrados", usuarioEditado.id);
      ...
      setUsuarios(usuarios.map(user => user.id === usuarioEditado.id ? usuarioEditado : user));
      ...
      Novo Usuário
          ...
          <TableContainer component={Paper}>
            <Table>
              <TableHead>
                <TableRow>
                  {['Nome', 'E-mail', 'CPF', 'Função', 'Ações'].map((headItem) => (
                    <TableCell key={headItem} style={{ fontWeight: 600 }}>
                      {headItem}
                    </TableCell>
                  ))}
                </TableRow>
              ...
              <TableBody>
                {usuarios.filter(usuario =>
                  ...
                  (usuario.funcao && usuario.funcao.toLowerCase().includes(termoPesquisa.toLowerCase()))
                ).map((usuario) => (
            ...
            >
            <DialogTitle className="DialogTitle">Cadastrar Novo Usuário</DialogTitle>
